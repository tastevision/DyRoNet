...
def main():
    ...
    # 得到所有的标注数据
    with open(opts.annot_path, "r") as f:
        temp_data = json.loads(f.read())

    for sid, seq in enumerate(tqdm(seqs)):
        scene_data = temp_data.copy()
        scene_data["images"] = [img for img in scene_data["images"] if img['sid'] == sid]
        with open("/tmp/my_temp_annot.json", "w") as f:
            f.write(json.dumps(
                scene_data,
                ensure_ascii=False,
                indent=None,
        ...
        scene_db = COCO("/tmp/my_temp_annot.json")
        results_ccf = []
        ...
        out_path = join(out_dir, f'results_ccf_{seq}.pkl')
        if opts.overwrite or not isfile(out_path):
            pickle.dump(results_ccf, open(out_path, 'wb'))

        out_path = join(out_dir, f'eval_assoc_{seq}.pkl')
        if opts.overwrite or not isfile(out_path):
            pickle.dump({
                'miss': miss,
                'in_time': in_time,
        ...
        if not opts.no_eval:
            eval_summary = eval_ccf(scene_db, results_ccf)
            out_path = join(out_dir, f'eval_summary_{seq}.pkl')
            if opts.overwrite or not isfile(out_path):
                pickle.dump(eval_summary, open(out_path, 'wb'))
            if opts.eval_mask:
                print('Evaluating instance segmentation')
                eval_summary = eval_ccf(scene_db, results_ccf, iou_type='segm')
                out_path = join(out_dir, 'eval_summary_mask_{seq}.pkl')
                if opts.overwrite or not isfile(out_path):
                    pickle.dump(eval_summary, open(out_path, 'wb'))
