...
class StreamDynamic_DAMO_LSN(nn.Module):
    """
    StreamDynamic DAMO-StreamNet和LongShortNet
    """
    ...
    def long_short_fusion(self, short_outs, long_outs, rurrent_pan_outs, jian0, jian1, jian2, branch):
        """
        long和short输出的融合过程
        """
        # print("---------------------------------------------------")
    ...
    def forward_process(self, x, N, buffer, test=False):
        ...
        if not test:
            ...
            fpn_outs = self.long_short_fusion(short_fpn_outs, long_fpn_outs, rurrent_pan_outs,
                                              self.jian0[N], self.jian1[N], self.jian2[N], branch=N)
    ...
    def compute_loss(self, fpn_outs, bbox_preds_t, obj_preds_t, cls_preds_t, x, targets, N):
        ...
        if isinstance(self.head[N], StudentHead):
    ...
    def forward_test_offline(self, x, targets=None, buffer=None, mode="max", branch=None):
        ...
        if branch is None:
            ...
            if mode == "max":
                ...
                N = np.random.randint(2) # 随机选择一个分支
            elif mode == "most":
        ...
        logger.info(f"process_time: {end - beg}")
        return output
