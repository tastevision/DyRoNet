...
class Exp(MyExp):
    def __init__(self):
        ...
        self.depth_t = 1
        self.width_t = 1
        self.backbone_depth = [0.33, 0.67, 1.0]
        self.backbone_width = [0.50, 0.75, 1.0]
        ...
        self.long_cfg = [
            ...
            dict(frame_num=3, delta=1, with_short_cut=False, out_channels=[((42, 85, 170), 2), ((44, 86, 172), 1)],),
        ...
        self.neck_cfg = [
            ...
            },
            {
                ...
                'in_channels': [
                    round(256 * (self.backbone_width[1] / 1.0)),
                    round(512 * (self.backbone_width[1] / 1.0)),
                    round(1024 * (self.backbone_width[1] / 1.0)),
                ],
                'out_channels': [
                    round(256 * (self.backbone_width[1] / 1.0)),
                    round(512 * (self.backbone_width[1] / 1.0)),
    ...
    def get_model(self):
        ...
        from exps.model.yolox_stream_dynamic import StreamDynamic
        from exps.model.dynamic_long_backbone import DynamicLongBackbone
        from exps.model.dynamic_short_backbone import DynamicShortBackbone
        from exps.model.neck_backbone import NeckBackbone 
        ...
        if getattr(self, "model", None) is None:
            ...
            long_backbone_s = nn.ModuleList([
                DynamicLongBackbone(depth, 
                                    width, 
                                    in_channels=in_channels, 
                                    frame_num=long_cfg["frame_num"],
                                    with_short_cut=long_cfg["with_short_cut"],
                                    out_channels=long_cfg["out_channels"])
                for depth, width, long_cfg in zip(self.backbone_depth, self.backbone_width, self.long_cfg)
            ])
            short_backbone_s = nn.ModuleList([
                DynamicShortBackbone(depth, 
                                     width, 
                                     in_channels=in_channels, 
                                     frame_num=short_cfg["frame_num"],
                                     with_short_cut=short_cfg["with_short_cut"],
                                     out_channels=short_cfg["out_channels"])
                for depth, width, short_cfg in zip(self.backbone_depth, self.backbone_width, self.short_cfg)
            ])
            backbone_neck_s = nn.ModuleList([
                NeckBackbone(depth, width, in_channels=in_channels, neck_cfg=neck_cfg)
                for depth, width, neck_cfg in zip(self.backbone_depth, self.backbone_width, self.neck_cfg)
            ])

            one_factor = 0.4
            head_s = nn.ModuleList([StudentHead(
                self.num_classes, width, in_channels=in_channels, gamma=1.0, 
            ...
            self.model = StreamDynamic(speed_detector,
                                       long_backbone_s, 
                                       short_backbone_s, 
                                       backbone_neck_s,
                                       head_s, 
    ...
    def get_data_loader(self, batch_size, is_distributed, no_aug=False, local_rank=0, cache_img=False):
        ...
        dataset = LONGSHORT_Dil_ARGOVERSEDataset(
            ...
            cache=cache_img,
            short_cfg=self.short_cfg[0],
            long_cfg=self.long_cfg[0],
        )
    ...
    def get_trainer(self, args):
        from exps.train_utils.longshort_dil_trainer_dynamic_3branch import Trainer
